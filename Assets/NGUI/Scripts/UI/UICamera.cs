...
/// <summary>
/// This script should be attached to each GetComponent<Camera>() that's used to draw the objects with
/// UI components on them. This may mean only one GetComponent<Camera>() (main GetComponent<Camera>() or your UI GetComponent<Camera>()),
/// or multiple cameras if you happen to have multiple viewports. Failing to attach this
/// script simply means that objects drawn by this GetComponent<Camera>() won't receive UI notifications:
/// 
/// * OnHover (isOver) is sent when the mouse hovers over a GetComponent<Collider>() or moves away.
/// * OnPress (isDown) is sent when a mouse button gets pressed on the GetComponent<Collider>().
...
/// 
/// * OnInput (text) is sent when typing (after selecting a GetComponent<Collider>() by clicking on it).
/// * OnTooltip (show) is sent when the mouse hovers over a GetComponent<Collider>() for some time without moving.
...
public class UICamera : MonoBehaviour
{
	...
	/// <summary>
	/// Event type -- use "UI" for your user interfaces, and "World" for your game GetComponent<Camera>().
	...
	/// <summary>
	/// Raycast range distance. By default it's as far as the GetComponent<Camera>() can see.
	/// </summary>
	...
	/// <summary>
	/// Various keys used by the GetComponent<Camera>().
	/// </summary>
	...
	/// Use UICamera.current to get the current GetComponent<Camera>().
	/// </summary>
	...
	/// <summary>
	/// Last GetComponent<Camera>() active prior to sending out the event. This will always be the GetComponent<Camera>() that actually sent out the event.
	/// </summary>
	...
	/// <summary>
	/// Convenience function that returns the main HUD GetComponent<Camera>().
	/// </summary>
	...
	static public bool Raycast (Vector3 inPos, out RaycastHit hit)
	{
		for (int i = 0; i < list.size; ++i)
		{
			...
			// If it's outside the GetComponent<Camera>()'s viewport, do nothing
	...
	/// <summary>
	/// Find the GetComponent<Camera>() responsible for handling events on objects of the specified layer.
	/// </summary>
	...
	/// <summary>
	/// Add this GetComponent<Camera>() to the list.
	/// </summary>
	...
	/// <summary>
	/// Remove this GetComponent<Camera>() from the list.
	/// </summary>
	...
	/// <summary>
	/// We don't want the GetComponent<Camera>() to send out any kind of mouse events.
	/// </summary>
	...
	public void ProcessMouse ()
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			// We don't want to update the last GetComponent<Camera>() while there is a touch happening

...
{
	/// <summary>
	/// Active GetComponent<Animation>() that resulted in the event notification.
	/// </summary>

	static public ActiveAnimation current;

	/// <summary>
	/// Event delegates called when the GetComponent<Animation>() finishes.
	/// </summary>
	...
	/// <summary>
	/// Whether the GetComponent<Animation>() is currently playing.
	/// </summary>
	...
	/// <summary>
	/// Manually reset the active GetComponent<Animation>() to the beginning.
	/// </summary>
	...
	/// <summary>
	/// Notify the target when the GetComponent<Animation>() finishes playing.
	/// </summary>
	...
	/// <summary>
	/// Play the specified GetComponent<Animation>().
	/// </summary>
	...
	void Play (string clipName, Direction playDirection)
	{
		if (mAnim != null)
		{
			// We will sample the GetComponent<Animation>() manually so that it works when the time is paused
			enabled = true;
			mAnim.enabled = false;
			...
			// Play the GetComponent<Animation>() if it's not playing already
			if (noName)
			{
			...
			// Update the GetComponent<Animation>() speed based on direction -- forward or back
			foreach (AnimationState state in mAnim)
			{
				if (string.IsNullOrEmpty(clipName) || state.name == clipName)
				{
					...
					// Automatically start the GetComponent<Animation>() from the end if it's playing in reverse
	...
	/// <summary>
	/// Play the specified GetComponent<Animation>() on the specified object.
	/// </summary>
